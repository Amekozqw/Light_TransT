...
class LightFeatureFusionNetwork(nn.Module):

    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
                 dim_feedforward=2048, dropout=0.1, activation="relu"):
        super().__init__()

        featurefusion_layer = LightFeatureFusionLayer(d_model, nhead, dim_feedforward, dropout, activation)
        self.encoder = Encoder(featurefusion_layer, num_featurefusion_layers)
    ...
    def restore_shape(self, src:Tensor, src_copy:Tensor):
        b, c, w, h = src_copy.size()
        src = src.permute(1, 2, 0).view(b, c, w, h)
        return src

    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        src_temp_copy = src_temp.clone()
        src_search_copy = src_search.clone()
        src_temp = src_temp.flatten(2).permute(2, 0, 1)
        pos_temp = pos_temp.flatten(2).permute(2, 0, 1)
        src_search = src_search.flatten(2).permute(2, 0, 1)
        pos_search = pos_search.flatten(2).permute(2, 0, 1)
        ...
        memory_temp, memory_search = self.restore_shape(memory_temp, src_temp_copy), \
                                        self.restore_shape(memory_search, src_search_copy)

        correlation_map = self.xcorr_pixelwise(memory_search, memory_temp)
        correlation_map = correlation_map.flatten(2).permute(2, 0, 1)

        hs = self.decoder(correlation_map, correlation_map,
                          tgt_key_padding_mask=mask_search,
                          memory_key_padding_mask=mask_search,
                          pos_enc=pos_search, pos_dec=pos_search)
        return hs.unsqueeze(0).transpose(1, 2)


class LightFeatureFusionLayer(nn.Module):
...
def build_lightfeaturefusion_network(settings):
    return LightFeatureFusionNetwork(
        d_model=settings.hidden_dim,
        dropout=settings.dropout,
        nhead=settings.nheads,
        dim_feedforward=settings.dim_feedforward,
