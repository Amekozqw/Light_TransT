...
class Backbone_light(BackboneBase):
    """ResNet backbone with frozen BatchNorm."""
    def __init__(self, settings, pretrained):
        backbone = backbones.mobilenetv3_small()
        if pretrained:
            self.load_pretrain(backbone, settings.pretrain_dir)
        num_channels = 48
        super().__init__(backbone, num_channels)
...
def build_backbone_light(settings, backbone_pretrained=True):
    position_embedding = build_position_encoding(settings)
    backbone = Backbone_light(settings, pretrained=backbone_pretrained)
    model = Joiner(backbone, position_embedding)
    model.num_channels = backbone.num_channels
    return model