...
from ltr.models.backbone.transt_backbone import build_backbone, build_backbone_light
from ltr.models.loss.matcher import build_matcher
from ltr.models.neck.featurefusion_network import build_featurefusion_network, build_lightfeaturefusion_network
...
def transt_mobilenet(settings):
    num_classes = 1
    backbone_net = build_backbone_light(settings, backbone_pretrained=True)
    featurefusion_network = build_lightfeaturefusion_network(settings)
    model = TransT(
        backbone_net,
        featurefusion_network,
        num_classes=num_classes
...
@model_constructor
def transt_resnet50_lightfeaturefusion(settings):
    num_classes = 1
    backbone_net = build_backbone(settings, backbone_pretrained=True)
    featurefusion_network = build_lightfeaturefusion_network(settings)
    model = TransT(
