...
def run(settings):
    ...
    settings.hidden_dim = 64 # 256 64
    settings.dropout = 0.1
    settings.nheads = 8
    settings.dim_feedforward = 1024
    settings.featurefusion_layers = 4

    settings.pretrain_dir = '/data/xlsun/code/TransT_light/ltr/models/pretrained/mobilenetv3_small_1.0.pth'
    settings.env.workspace_dir = '/data/xlsun/code/TransT_light/results/TransT_light_500e_bs76'
    ...
    #                         samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train)
    dataset_train = sampler.TransTSampler([got10k_train, coco_train], [1, 1],
                            samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train)
