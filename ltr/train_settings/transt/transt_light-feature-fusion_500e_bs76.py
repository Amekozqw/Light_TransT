import torch
from ltr.dataset import Lasot, MSCOCOSeq, Got10k, Got10k_SSHClient, TrackingNet
...
def run(settings):
    ...
    settings.hidden_dim = 256 # 256 64
    settings.dropout = 0.1
    settings.nheads = 8
    settings.dim_feedforward = 1024
    settings.featurefusion_layers = 6

    settings.pretrain_dir = '/data01/xlsun/code/TransT_light/ltr/models/pretrained/mobilenetv3_small_1.0.pth'
    settings.env.workspace_dir = '/data01/xlsun/code/TransT_light/results/transt_light-feature-fusion_500e_bs76'
    ...
    # ssh_client_info = {'hostname':'10.88.206.213', 'port':30704, 'username':'ubuntu', 'password':'nanhu@2021'}
    # got10k_train = Got10k_SSHClient(settings.env.got10k_ssh_dir, split='vottrain', ssh_client_info=ssh_client_info)
    ...
    # dataset_train = sampler.TransTSampler([got10k_train, lasot_train, coco_train], [1, 1, 1],
    #                         samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train)
    dataset_train = sampler.TransTSampler([got10k_train, lasot_train, coco_train], [1, 1, 1],
                            samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train)
    ...
    model = transt_models.transt_resnet50_lightfeaturefusion(settings)
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [400, 800])
